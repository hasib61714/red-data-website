export default function ServiceCard({ icon, title, description, tag, onClick }) {
  return (
    <div
      onClick={onClick}
      className="group relative h-full flex flex-col bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-white/[0.08] rounded-2xl p-5 hover:border-red-500/30 hover:shadow-xl hover:shadow-red-500/5 hover:-translate-y-1 transition-all duration-300 cursor-pointer overflow-hidden"
    >
      ...
      <div className="flex items-start gap-5 flex-1">
        <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-700/60 flex items-center justify-center text-xl group-hover:bg-red-500/20 group-hover:scale-110 transition-all duration-300">
          {icon}
        </div>
