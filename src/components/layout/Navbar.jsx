                  ...
                  <div className="p-3">
                    ...
                    <div className="grid grid-cols-2 gap-0.5">
                      {serviceItems.map((item) => (
                        ...
                        >
                          <span className="flex-shrink-0 w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/[0.07] flex items-center justify-center text-slate-500 group-hover:bg-red-50 dark:group-hover:bg-red-500/15 group-hover:border-red-200 dark:group-hover:border-red-500/20 group-hover:text-red-500 transition-all duration-150">
        ...
        <div className={`xl:hidden overflow-hidden transition-all duration-300 ease-in-out ${menuOpen ? 'max-h-[860px] opacity-100 pb-4' : 'max-h-0 opacity-0'}`}>
          ...
          <nav className="flex flex-col gap-0.5">
            ...
            <Link to="/corporate-internet" onClick={() => setMenuOpen(false)}
              ...
              <span className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 flex-shrink-0"><Icon name="building" className="w-4 h-4" /></span>
              {T('nav.corporateInternet')}
            </Link>

            <Link to="/home-internet" onClick={() => setMenuOpen(false)}
              className={mobileLinkCls(pathname === '/home-internet')}>
              <span className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 flex-shrink-0"><Icon name="wifi" className="w-4 h-4" /></span>
              {T('nav.homeInternet')}
            </Link>
            ...
            <div>
              <button onClick={() => setMobileServicesOpen(v => !v)}
                ...
                <span className="flex items-center gap-2.5">
                  <span className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 flex-shrink-0"><Icon name="layers" className="w-4 h-4" /></span>
                  {T('nav.services')}
                </span>
                ...
                <div className="ml-4 mt-1 pl-3 border-l border-slate-200 dark:border-white/[0.08] flex flex-col gap-0.5 pb-1">
                  {serviceItems.map(item => (
                    <Link key={item.label} to={item.to}
                      ...
                      <span className="flex-shrink-0 w-5 h-5 text-slate-500"><Icon name={item.icon} className="w-4 h-4" /></span>
                      <div>
            ...
            <Link to="/about-us" onClick={() => setMenuOpen(false)}
              ...
              <span className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 flex-shrink-0"><Icon name="info" className="w-4 h-4" /></span>
              {T('nav.aboutUs')}
            </Link>

            <Link to="/contact" onClick={() => setMenuOpen(false)}
              className={mobileLinkCls(pathname === '/contact')}>
              <span className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 flex-shrink-0"><Icon name="mail" className="w-4 h-4" /></span>
              {T('nav.contactUs')}
            </Link>
          </nav>
